...
#define TFT_DISPLAY_VTAR_GAIN_NUMERATOR_BUCK     1000U
#define TFT_DISPLAY_VTAR_GAIN_NUMERATOR_BOOST     998U
/* BUCK 下 II/IIN 基于当前板固定点位重拟合：Ii_real ≈ 1.083 * Ii_display + 0.25A */
#define TFT_DISPLAY_IX_LINEAR_NUMERATOR_BUCK     1083U
#define TFT_DISPLAY_IX_LINEAR_DENOMINATOR_BUCK   1000U
#define TFT_DISPLAY_IX_LINEAR_ROUNDING_BUCK      500U
#define TFT_DISPLAY_IX_LINEAR_OFFSET_BUCK        25
...
static void TftDebugView_DrawMainValues(void)
{
  ...
  else
  {
    ...
    ix_display_current = (uint16_t)((((uint32_t)ix_display_current * TFT_DISPLAY_IX_GAIN_NUMERATOR_BUCK) +
                                     TFT_DISPLAY_GAIN_ROUNDING) /
                                    TFT_DISPLAY_GAIN_DENOMINATOR);
    ix_display_current = (uint16_t)((((uint32_t)ix_display_current * TFT_DISPLAY_IX_LINEAR_NUMERATOR_BUCK) +
                                     TFT_DISPLAY_IX_LINEAR_ROUNDING_BUCK) /
                                    TFT_DISPLAY_IX_LINEAR_DENOMINATOR_BUCK);
    ix_display_current = (uint16_t)(ix_display_current + TFT_DISPLAY_IX_LINEAR_OFFSET_BUCK);
