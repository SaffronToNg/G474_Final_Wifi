...
#define TFT_DISPLAY_IX_GAIN_NUMERATOR_BOOST      1088U
#define TFT_DISPLAY_IY_GAIN_NUMERATOR_BOOST      1170U
#define TFT_DISPLAY_IY_GAIN_NUMERATOR_BUCK       1360U
#define TFT_DISPLAY_VTAR_GAIN_NUMERATOR_BUCK      981U
/* BUCK 下 IY 基于最新实测点做轻量线性补偿：Iy_real ≈ 0.905 * Iy_display + 0.25A */
#define TFT_DISPLAY_IY_LINEAR_NUMERATOR_BUCK     905U
#define TFT_DISPLAY_IY_LINEAR_DENOMINATOR_BUCK   1000U
#define TFT_DISPLAY_IY_LINEAR_ROUNDING_BUCK      500U
#define TFT_DISPLAY_IY_LINEAR_OFFSET_BUCK        25U
...
static void TftDebugView_DrawMainValues(void)
{
  ...
  {
    iy_display_current = (uint16_t)((((uint32_t)iy_display_current * TFT_DISPLAY_IY_GAIN_NUMERATOR_BUCK) +
                                     TFT_DISPLAY_GAIN_ROUNDING) /
                                    TFT_DISPLAY_GAIN_DENOMINATOR);
    iy_display_current = (uint16_t)((((uint32_t)iy_display_current * TFT_DISPLAY_IY_LINEAR_NUMERATOR_BUCK) +
                                     TFT_DISPLAY_IY_LINEAR_ROUNDING_BUCK) /
                                    TFT_DISPLAY_IY_LINEAR_DENOMINATOR_BUCK);
    iy_display_current = (uint16_t)(iy_display_current + TFT_DISPLAY_IY_LINEAR_OFFSET_BUCK);
    ...
    {
      uint16_t iy_delta = (iy_display_current > s_last_iy_display) ?
                              (uint16_t)(iy_display_current - s_last_iy_display) :
                              (uint16_t)(s_last_iy_display - iy_display_current);
      if (iy_delta <= TFT_DISPLAY_STABLE_DEADBAND_BUCK)
      {
        iy_display_current = s_last_iy_display;
      }
    }
  }
